...
class CVAE(torch.nn.Module):
    def __init__(self):
        ...
        self.mu = torch.nn.Conv2d(in_channels=128, out_channels=code_dim, kernel_size=3, stride=1, padding=0)  # 2*1*1
        self.logvar = torch.nn.Conv2d(in_channels=128, out_channels=code_dim, kernel_size=3, stride=1, padding=0)  # 2*1*1

        self.decoder = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(in_channels=code_dim, out_channels=128, kernel_size=3, stride=1, padding=0),  # 3*3
            torch.nn.ReLU(),
...
def scatter(feat, label, epoch):
    ...
    plt.draw()
    plt.savefig('./cvae/scatter_{}.png'.format(epoch))
    plt.pause(0.001)
...
def test(epoch):
    ...
    for i, (data, target) in enumerate(test_loader):
        ...
        recon_batch, feat, logvar = model(data)
        ...
        if i == 0:
            ...
            save_image(comparison.data.cpu(),
                       './cvae/reconstruction_' + str(epoch) + '.png', nrow=n)

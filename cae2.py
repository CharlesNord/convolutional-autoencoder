...
class CAE(torch.nn.Module):
    def __init__(self):
        ...
        self.encoder = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Conv2d(in_channels=128, out_channels=2, kernel_size=3, stride=1, padding=0)  # 1*1
        )
        self.decoder = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(in_channels=2, out_channels=128, kernel_size=3, stride=1, padding=0),  # 3*3
            torch.nn.ReLU(),
...
def test(epoch):
    ...
    for i, (data, target) in enumerate(test_loader):
        ...
        feat_total.append(feat.data.cpu().view(-1, 2))
